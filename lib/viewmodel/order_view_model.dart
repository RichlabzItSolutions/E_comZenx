...
class OrderViewModel extends ChangeNotifier {
  ...
  List<Order> get filteredOrders {
    ...
    if (_selectedTab == 'Active') {
      List<String> activeStatuses = status.split(','); // Split '1,2,3' into a list ['1', '2', '3']
      final filtered = _orders.where((order) =>
      activeStatuses.contains(order.orderStatus.toString()) &&
          order.orderStatus != 4 && order.orderStatus != 5).toList();
      return filtered;
    } else {
      final filtered = _orders.where((order) => order.orderStatus.toString() == status).toList();
      return filtered;
    }
  }
    ...
    try {
      ...
      if (userIdString != null) {
        ...
        if (_selectedTab == 'Active') {
          ...
          fetchedOrders = await authService.fetchOrders(userId, "1"); // Pass the status as a comma-separated string
        } else {
